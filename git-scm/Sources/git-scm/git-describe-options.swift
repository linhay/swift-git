...
static let always: Self = "--always"
    ///  --broken[=<mark>]
    static func broken(mark: [String]) -> Self { .init("--broken=\(mark.joined(separator: ","))") }
    ///  --candidates=<n>
    static func candidates(n: String) -> Self { .init("--candidates=\(n)") }
    ///  --contains
static let contains: Self = "--contains"
    ///  --debug
static let debug: Self = "--debug"
    ///  --dirty[=<mark>]
    static func dirty(mark: [String]) -> Self { .init("--dirty=\(mark.joined(separator: ","))") }
    ///  --exact-match
static let exactMatch: Self = "--exact-match"
    ///  --exclude <pattern>
    static func exclude(pattern: String) -> Self { .init("--exclude \(pattern)") }
    ///  --first-parent
static let firstParent: Self = "--first-parent"
    ///  --long
static let long: Self = "--long"
    ///  --match <pattern>
    static func match(pattern: String) -> Self { .init("--match \(pattern)") }
    ///  --tags
