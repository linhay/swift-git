...
static let branch: Self = "--branch"
    ...
    ///  --ignored[=<mode>]
    static func ignored(mode: [String]) -> Self { .init("--ignored=\(mode.joined(separator: ","))") }
    ///  --long
...
static let noRenames: Self = "--no-renames"
    ///  --porcelain[=<version>]
    static func porcelain(version: [String]) -> Self { .init("--porcelain=\(version.joined(separator: ","))") }
    ///  --renames
