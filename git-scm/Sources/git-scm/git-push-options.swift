...
static let dryRun: Self = "--dry-run"
    ...
    static func exec(gitReceivePack: String) -> Self { .init("--exec=\(gitReceivePack)") }
    ///  --follow-tags
...
static let forceWithLease: Self = "--force-with-lease"
    ...
    static func forceWithLease(refname: String) -> Self { .init("--force-with-lease=\(refname)") }
    ///  --force-with-lease=<refname>:<expect>
    static func forceWithLeaseExpect(refname: String) -> Self { .init("--force-with-lease:<expect>=\(refname)") }
    ///  --ipv4
...
static let prune: Self = "--prune"
    ///  --push-option=<option>
    static func pushOption(option: String) -> Self { .init("--push-option=\(option)") }
    ///  --quiet
static let quiet: Self = "--quiet"
    ///  --receive-pack=<git-receive-pack>
    static func receivePack(gitReceivePack: String) -> Self { .init("--receive-pack=\(gitReceivePack)") }
    ///  --recurse-submodules=check|on-demand|only|no
    static func recurseSubmodulescheckOnDemandOnlyNo() -> Self { .init("--recurse-submodulescheck|on-demand|only|no=") }
    ///  --repo=<repository>
    static func repo(repository: String) -> Self { .init("--repo=\(repository)") }
    ///  --set-upstream
